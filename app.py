...
CORRECTION_SYSTEM_PROMPT = (
    "You are an expert music lyrics editor. Your job is to output the COMPLETE, "
    "accurate official lyrics of a song.\n\n"
    "You are given a speech-to-text transcription of the song, but treat it as "
    "an UNRELIABLE HINT only: it frequently mishears words and often drops "
    "entire lines, verses, or whole sections. It must never be your primary "
    "source.\n\n"
    "Source priority:\n"
    "1. PRIMARY SOURCE: the web. When a web_search tool is available you MUST "
    "use it to find the official lyrics, and base your answer on what you find. "
    "Search '<song> <artist> lyrics' (run more searches if the first is weak). "
    "Reproduce the real lyrics IN FULL, including every verse, chorus "
    "repetition, bridge, and any lines the transcription is missing.\n"
    "2. FALLBACK: only if web search is unavailable, fails, or returns nothing "
    "usable, fall back to your own knowledge of the song, and only then to the "
    "transcription text.\n\n"
    "Use the transcription only to identify the song/version, to choose between "
    "ambiguous results, and as a last-resort fallback for parts you cannot find "
    "online. Do NOT limit the output to the lines present in the transcription.\n\n"
    "Rules:\n"
    ...
    "- Remove ASR artifacts; do not let the transcription's missing or garbled "
    "lines shrink the real lyrics. Restore the full official text.\n"
    "- Do not fabricate lyrics: every line must come from a reputable source, "
    "your knowledge of the song, or (last resort) the transcription.\n"
...
def correct_lyrics_with_nova(song_name: str, lyrics: str) -> tuple[str, list[str]]:
    ...
    user_text = (
        f"{song_line}\n\n"
        "Find and return the COMPLETE official lyrics for this song. Use the "
        "web_search tool as your primary source. The transcription below is an "
        "unreliable hint only (it may be missing lines or whole sections) — use "
        "it to identify the song and as a fallback, NOT as the main source.\n\n"
        "--- BEGIN UNRELIABLE TRANSCRIPTION HINT ---\n"
        f"{lyrics}\n"
        "--- END UNRELIABLE TRANSCRIPTION HINT ---\n\n"
        "Return only the formatted, complete lyrics."
    )
